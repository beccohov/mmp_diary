...
Но я пока оставлю эту идею, чтобы далее проверить больше гипотез (время работы GPU ограничено :( ). 
 ...
 Также обнаружил странный факт - при попытке обучения другой схожей модели для распознования эмоций на английском языке, но предобученной только на английском (wav2vec2 large-960h) отсутствует сходимость.
 </li>
 <li>
 <b>18.04.22</b> - Реализовал метод аугментации, описанный ранее (замена с вероятностью p1 части аудио (с конца) другим аудио (случайной длительности с вероятностью p2 заполнения всей записи)). Провожу эксперименты. Из проведенного на фолде 0 (на нем самая низкая точность) эксперимента следует, что точность улучшается, однако на остальных фолдах похоже, что немного снижается. Чтобы снизить дисперсию точности при выходе на асимптоту. Также нашел статью про паралингвистические эмбеддинги. Но к сожалению там весь код на tf.
  </li>
 </ul>
 <br>
